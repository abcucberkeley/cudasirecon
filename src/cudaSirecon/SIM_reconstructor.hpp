...
private:
  ...
  #ifdef MRC
  IW_MRC_HEADER m_out_header;
  IW_MRC_HEADER m_in_header;
  #endif
  ...
  //! Apply the (optional) bounding-box crop to rawImage for TIFF inputs.
  /*!
    Called from setFile() after the raw TIFF has been loaded. Does nothing if
    m_myParams.bCropBBox is false. When enabled, rebuilds rawImage so that
    it has the post-crop dimensions (nx_raw x ny x nz_cropped*nphases*ndirs,
    preserving the fastSIM / non-fastSIM z-section interleaving and all
    wavelength channels).
   */
