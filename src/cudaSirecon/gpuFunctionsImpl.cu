    ...
    int norders, std::vector<GPUBuffer>*rawImages, float *sepMatrix)
{
  ...
  std::vector<float*> output(nphases);
  int xdim = (nx/2 + 1) * 2;
    ...
    std::vector<GPUBuffer>* OTF, short wave, ReconParams* pParams)
{
  ...
  float kzscale = dkz / pParams->dkzotf;
  float rdistcutoff = pParams->na * 2.0 / (wave * 0.001);
  if (rdistcutoff > 1./(2.*dxy) ) rdistcutoff = 1. / (2.*dxy);

  //! k0 magnitude (of the lowest order) in 1/micron
  float k0mag = sqrt(k0x * k0x + k0y * k0y);
  float lambdaem = (wave / pParams->nimm) / 1.0e3;
  float lambdaexc = 0.88 * lambdaem;
  float alpha = asinf(pParams->na / pParams->nimm);
  ...
  if (!pParams->bTwolens && !pParams->bBessel) {
    ...
    float halfangle;
    float kzExMax;
    kzExMax = 2.0 * pParams->BesselNA / pParams->BesselLambdaEx;
    halfangle = acosf(k0mag * order2 /(pParams->norders-1)/ kzExMax);
  ...
  float otfcutoff = pParams->otfcutoff;
  if (pParams->bBessel)
    otfcutoff = 0.01;
  ...
  cutilSafeCall(cudaMemcpyToSymbol(const_otfPtrs,
        &otfPtrs[0], pParams->norders * sizeof(cuFloatComplex *),
        0, cudaMemcpyHostToDevice));
  
  //float2 *a2[10] ={0};
  //cudaError_t err2 = cudaMemcpyFromSymbol(a2, const_otfPtrs, sizeof(float2*) * 2);
  //printf(" % s\n", cudaGetErrorString(err2));

  //int a6[10] = { 1,2,3 };
  //int a7 = 222;
  //int a8 = 8;
  //err2 = cudaMemcpyFromSymbol(&a7, const_pParams_bNoKz0, sizeof(int));
  //printf(" % s\n", cudaGetErrorString(err2));
  //err2 = cudaMemcpyFromSymbol(&a8, const_pParams_suppression_radius, sizeof(int));
  //printf(" % s\n", cudaGetErrorString(err2));

  //float2 a5[10] = { 0 };
  //cudaError_t err5 = cudaMemcpy(a5, (a2[0]), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
  //printf(" % s\n", cudaGetErrorString(err5));

  //float2 a3[10] = { 0 };
  //cudaError_t err3 = cudaMemcpy(a3, (bands->at(0).getPtr()), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
  ...
  if (j<nx) {
    ...
    if (rdist1 <= rdistcutoff) {
      ...
      if (rdist12 <= rdistcutoff || rdist21 <= rdistcutoff) { 
        ...
        int tt = const_pParams_suppression_radius;
        int tt2 = const_pParams_bNoKz0;
    ...
    short wave, ReconParams* pParams)
{
  ...
  //int a7 = 222;
  //int a8 = 8;
  //cudaError_t err2 = cudaMemcpyFromSymbol(&a7, const_pParams_bNoKz0, sizeof(int));
  //printf(" % s\n", cudaGetErrorString(err2));
  //err2 = cudaMemcpyFromSymbol(&a8, const_pParams_suppression_radius, sizeof(int));
  //printf(" % s\n", cudaGetErrorString(err2));
  ...
  for (order=0;order<norders;order++) {
    if (order==0) {
      ...
      //dev_bandptr2 = 0
      dev_bandptr2 = (cuFloatComplex*)bands->at(0).getPtr();
    }
    else {
    ...
    //cudaError_t err3 = cudaMemcpy(a3, (bands->at(0).getPtr()), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
    //printf(" % s\n", cudaGetErrorString(err3));
    //err3 = cudaMemcpy(a4, (bands->at(1).getPtr()), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
    //printf(" % s\n", cudaGetErrorString(err3));
    ...
    //float2 ccpu[arraySize] = { 0 };
    //float2 ccpu2[arraySize] = { 0 };
    //cudaError_t err2 = cudaMemcpy(ccpu, dev_bandptr, sizeof(float2) * 5, cudaMemcpyDeviceToHost);
    //printf("1 % s\n", cudaGetErrorString(err2));
    //err2 = cudaMemcpy(ccpu2, dev_bandptr2, sizeof(float2) * 5, cudaMemcpyDeviceToHost);
    //printf("2 % s\n", cudaGetErrorString(err2));
									...
									cuFloatComplex * dev_bandptr, cuFloatComplex * dev_bandptr2, bool bSecondEntry)
{
  ...
  if (x1 <= nx/2) {
    ...
    if (rdist1<=rdistcutoff) { /* is x1,y1 within the theoretical lateral OTF support of 
      ...
      if (order == 0 && z0 == 0 && const_pParams_bNoKz0)
      {
          //std::cout << "order == 0 bNoKz0" << std::endl;
          dampfact = 0;
      }
...
__device__ float dev_order0damping(float radius, float zindex, float rlimit, int zlimit)
{
  ...
  float b = (float)zlimit + 0.000001;
  zfraction = fabs(zindex/b);
    ...
    float dxy, float zoomfact, int z_zoom, float expfact)
{
  ...
  move_kernel<<<grid,block>>>(
    ...
    cudaError_t err3 = cudaMemcpy(a3, (bands->at(0).getPtr()), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
    printf(" % s\n", cudaGetErrorString(err3));
    err3 = cudaMemcpy(a4, (bigbuffer->getPtr()), sizeof(float2) * 10, cudaMemcpyDeviceToHost);
    printf(" % s\n", cudaGetErrorString(err3));
