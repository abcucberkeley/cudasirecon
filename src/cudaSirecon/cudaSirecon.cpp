...
void SetDefaultParams(ReconParams *pParams)
{
  ...
  pParams->bSuppress_singularities = 0;  /* whether to dampen the OTF values near band centers; used in filterbands() */
  ...
  pParams->readoutNoiseVar = 1;  // electron^2
  ...
  pParams -> bTIFF = false;
  pParams -> bOutputUint16 = false;

  // Optional bounding-box crop (TIFF inputs only). -1 means "not set";
  // bCropBBox is true only when all six coordinates are supplied.
  pParams->cropXmin = -1;
  pParams->cropXmax = -1;
  pParams->cropYmin = -1;
  pParams->cropYmax = -1;
...
//#endif
    ...
    clock_t End = clock();//??????
    std::cout << "Time for FFT: " << (double)(End - Begin) / CLOCKS_PER_SEC << "s" << std::endl;
      ...
      for (int phase = 0; phase < params->nphases; ++ phase) {
        ...
        for (int z = 0; z < imgParams.nz; ++z) {
          ...
          IMWrSec(separated_stream_no,
              imgPtr + (z + zoffset) * (imgParams.nx/2 + 1)*2 * imgParams.ny, (imgParams.nx / 2 + 1) * 2 * imgParams.ny, 2);
        }
      }
    ...
    if (params->bSearchforvector &&
      ...
      if (params->bSaveOverlaps) {
        ...
        if (params->bTIFF) {
          ...
          ovlp0.save_tiff(params->fileOverlaps.c_str(), 1);
        }
        #ifdef MRC
        else {
          ...
          for (int z = 0; z < imgParams.nz0; ++z) {
            IMWrSec(overlaps_stream_no, ol0Ptr + z * imgParams.nx * imgParams.ny, imgParams.nx* imgParams.ny,2);
            IMWrSec(overlaps_stream_no, ol1Ptr + z * imgParams.nx * imgParams.ny, imgParams.nx* imgParams.ny,2);
          }
        }
        #endif
      }
...
void dumpBands(std::vector<GPUBuffer>* bands, int nx, int ny, int nz0)
{
  ...
  for (auto i = bands->begin(); i != bands->end(); ++i) {
    ...
    band_tiff.save_tiff(s.str().c_str(), 1);
...
SIM_Reconstructor::SIM_Reconstructor(int argc, char **argv)
{
  ...
  // Extract config file path before calling notify(), so that config file
  // values can override defaults. Calling notify() first would populate the
  // variable map with defaults, causing store() to skip config file entries.
  if (m_varsmap.count("config"))
    m_config_file = m_varsmap["config"].as<std::string>();
  ...
  if (m_myParams.ifiles.size() >= 4 &&
      m_myParams.ifiles.compare(m_myParams.ifiles.size() - 4, 4, ".tif") == 0) {
      m_myParams.bTIFF = true;
      m_all_matching_files.push_back(m_myParams.ifiles);
  }
...
int SIM_Reconstructor::setupProgramOptions()
{
  ...
  m_progopts.add_options()
    ...
    ("nokz0", po::bool_switch(&m_myParams.bNoKz0), "not using kz=0 plane of the 0th order in the final assembly (mostly for debug)")
     ("apodizeoutput", po::value<int>(&m_myParams.nphases)->default_value(1))
    ...
    // Bounding-box crop for TIFF inputs. All six must be supplied together
    // (they can all go in the config file or all on the command line). Values
    // are 0-indexed and inclusive. After cropping, processing proceeds as if
    // that sub-region were the original raw data.
    ("cropXmin", po::value<int>(&m_myParams.cropXmin)->default_value(-1),
     "Crop bounding box x min (0-indexed, inclusive, TIFF only); requires all 6 crop{X,Y,Z}{min,max}")
    ("cropXmax", po::value<int>(&m_myParams.cropXmax)->default_value(-1),
     "Crop bounding box x max (0-indexed, inclusive, TIFF only)")
    ("cropYmin", po::value<int>(&m_myParams.cropYmin)->default_value(-1),
     "Crop bounding box y min (0-indexed, inclusive, TIFF only)")
    ("cropYmax", po::value<int>(&m_myParams.cropYmax)->default_value(-1),
     "Crop bounding box y max (0-indexed, inclusive, TIFF only)")
    ("cropZmin", po::value<int>(&m_myParams.cropZmin)->default_value(-1),
    ...
    ("uint16", po::bool_switch(&m_myParams.bOutputUint16),
     "Write TIFF output as uint16 instead of float32 (clamps negatives to 0 and values above 65535)")
...
int SIM_Reconstructor::setParams()
{
  ...
  if (m_varsmap.count("apodizeoutput")) {
    m_myParams.apodizeoutput = m_varsmap["apodizeoutput"].as<std::int32_t>();
    std::cout << "gamma = " << m_myParams.apoGamma << std::endl;
  }
  if (m_varsmap.count("nosuppress")) {
      m_myParams.bSuppress_singularities = 1-m_varsmap["nosuppress"].as<std::int32_t>();
  }
  ...
  // Bounding-box crop: each axis is independent. For a given axis, both
  // *min and *max must be supplied together (or neither). Any axis left
  // unspecified is filled in with the full raw-TIFF range once setup() has
  // read the file header. bCropBBox is true iff at least one axis is
  // explicitly cropped.
  {
    auto check_pair = [](int lo, int hi, const char* axis) {
      const bool lo_set = lo >= 0;
      const bool hi_set = hi >= 0;
      if (lo_set != hi_set) {
        std::stringstream ss;
        ss << "Crop bounding box: crop" << axis << "min and crop" << axis
           << "max must be specified together (got "
           << (lo_set ? std::to_string(lo) : std::string("unset")) << " / "
           << (hi_set ? std::to_string(hi) : std::string("unset")) << ").";
        throw std::runtime_error(ss.str());
      }
      if (lo_set && hi < lo) {
        std::stringstream ss;
        ss << "Crop bounding box: crop" << axis << "max (" << hi
           << ") must be >= crop" << axis << "min (" << lo << ").";
        throw std::runtime_error(ss.str());
      }
      return lo_set;  // true when this axis is being cropped
    };
    const bool xSet = check_pair(m_myParams.cropXmin, m_myParams.cropXmax, "X");
    const bool ySet = check_pair(m_myParams.cropYmin, m_myParams.cropYmax, "Y");
    const bool zSet = check_pair(m_myParams.cropZmin, m_myParams.cropZmax, "Z");
    m_myParams.bCropBBox = xSet || ySet || zSet;
    if (m_myParams.bCropBBox) {
      std::cout << "Crop bbox enabled (0-indexed, inclusive):";
      if (xSet) std::cout << " x[" << m_myParams.cropXmin
                          << ".." << m_myParams.cropXmax << "]";
      else      std::cout << " x[full]";
      if (ySet) std::cout << " y[" << m_myParams.cropYmin
                          << ".." << m_myParams.cropYmax << "]";
      else      std::cout << " y[full]";
      if (zSet) std::cout << " z[" << m_myParams.cropZmin
                          << ".." << m_myParams.cropZmax << "]";
      else      std::cout << " z[full]";
      std::cout << std::endl;
    }
  }
...
void SIM_Reconstructor::openFiles() {
  ...
  if (m_myParams.otffiles.size() >= 4 &&
      m_myParams.otffiles.compare(m_myParams.otffiles.size()-4, 4, ".tif") == 0) {
    m_otf_tiff.assign(m_myParams.otffiles.c_str());
    m_bOTFisTIFF = true;
    m_OTFfile_valid = true;
  }
...
void SIM_Reconstructor::setup()
{
  if (m_myParams.bTIFF) {
    ...
    // If a bounding-box crop was requested on any axis, fill in the axes the
    // user didn't supply with the full raw range, validate, then replace
    // nx_raw / ny / nz with the cropped sizes so that every downstream
    // allocation / index computation sees the cropped volume.
    if (m_myParams.bCropBBox) {
      if (m_myParams.cropXmin < 0) { m_myParams.cropXmin = 0; m_myParams.cropXmax = m_imgParams.nx_raw - 1; }
      if (m_myParams.cropYmin < 0) { m_myParams.cropYmin = 0; m_myParams.cropYmax = m_imgParams.ny     - 1; }
      if (m_myParams.cropZmin < 0) { m_myParams.cropZmin = 0; m_myParams.cropZmax = m_imgParams.nz     - 1; }

      if (m_myParams.cropXmax >= m_imgParams.nx_raw ||
          m_myParams.cropYmax >= m_imgParams.ny     ||
          m_myParams.cropZmax >= m_imgParams.nz) {
        std::stringstream ss;
        ss << "Crop bounding box out of range: TIFF logical dims ("
           << m_imgParams.nx_raw << "," << m_imgParams.ny << "," << m_imgParams.nz
           << "); bbox x[" << m_myParams.cropXmin << ".." << m_myParams.cropXmax
           << "] y[" << m_myParams.cropYmin << ".." << m_myParams.cropYmax
           << "] z[" << m_myParams.cropZmin << ".." << m_myParams.cropZmax << "]";
        throw std::runtime_error(ss.str());
      }

      printf("Applying crop bbox x[%d..%d] y[%d..%d] z[%d..%d] on TIFF (%d x %d x %d)\n",
             m_myParams.cropXmin, m_myParams.cropXmax,
             m_myParams.cropYmin, m_myParams.cropYmax,
             m_myParams.cropZmin, m_myParams.cropZmax,
             m_imgParams.nx_raw, m_imgParams.ny, m_imgParams.nz);

      m_imgParams.nx_raw = m_myParams.cropXmax - m_myParams.cropXmin + 1;
      m_imgParams.ny     = m_myParams.cropYmax - m_myParams.cropYmin + 1;
      m_imgParams.nz     = m_myParams.cropZmax - m_myParams.cropZmin + 1;
    }
  }
#ifdef MRC
  else 
    ::loadHeader(m_myParams, &m_imgParams, m_in_header);
  memcpy(&m_out_header, &m_in_header, sizeof(m_out_header));
#endif
...
//    m_imgParams.nx = m_imgParams.ny;
  ...
  if (!m_myParams.bTIFF) {
    if (m_myParams.bSaveAlignedRaw) {
      memcpy(&aligned_header, &m_out_header, sizeof(m_out_header));
    ...
    if (m_myParams.bSaveSeparated) {
      memcpy(&sep_header, &m_out_header, sizeof(m_out_header));
    ...
    if (m_myParams.bSaveOverlaps) {
      memcpy(&overlaps_header, &m_out_header, sizeof(m_out_header));
  ...
  //m_imgParams.inscale = 1.0e-3; //20240604 MQ
  ...
  if (m_imgParams.nz > 1) {
    ...
    k0magguess /= m_myParams.norders - 1;
  }
                                          ...
                                          int new_nz) const
{
  ...
  for (int w = 0; w < nwaves; ++w) {
    for (int dir_ = 0; dir_ < ndirs; ++dir_) {
      for (int z_ = 0; z_ < new_nz; ++z_) {
        const int orig_z = z_ + zmin;
...
void SIM_Reconstructor::setFile(int it, int iw)
{
  if (m_myParams.bTIFF) {
    ...
    CImg<uint16_t> rawImage_t;
    rawImage_t.assign(m_imgParams.nx_raw, m_imgParams.ny, m_imgParams.nz * m_myParams.nphases * m_myParams.ndirs);
    uint16_t* temp = rawImage_t.data(0, 0, 0);
    uint16_t* temp2 = rawImage_t.data(0, 0, 1);
    uint16_t* temp3 = rawImage_t.data(0, 0, 2);
    /*float* a = rawImage.data(0, 0, 0);
    uint16_t* a2 = rawImage_t.data(0, 0, 0);*/
    for (unsigned sec=0; sec<rawImage.depth(); sec++) {

      //IMPosnZWT(istream_no, sec, iw, it);
      IMRdSec(istream_no, rawImage_t.data(0, 0, sec), m_imgParams.nx_raw*m_imgParams.ny, m_in_header.mode);
    }
    ...
    for (z = 0; z < m_imgParams.nz; ++z) {
      ...
      for (int phase = 0; phase < m_myParams.nphases; ++phase) {
        ...
        if (m_myParams.bBgInExtHdr) {
          ...
          throw std::runtime_error("Extended header is not supported!");
          int extInts;
          float extFloats[3];
      ...
      for (int i = 0; i < m_myParams.norders; i++) {
        ...
        if (m_imgParams.nz == 1 || m_myParams.bRadAvgOTF) {
          if (otfheader.nz > i + dir*m_myParams.norders) {
            // each section in OTF file is OTF of one order; so load that section into otf[dir][i]
            IMRdSec(otfstream_no, otfTmp.getPtr(), otfTmp.getSize()/sizeof(cuFloatComplex), otfheader.mode);
...
void SIM_Reconstructor::writeResult(int it, int iw)
{
  ...
  CImg<> outCimg((float*) outbufferHost.getPtr(),
    m_myParams.zoomfact * m_imgParams.nx,
    m_myParams.zoomfact * m_imgParams.ny,
    m_myParams.z_zoom * m_imgParams.nz0, true);

  std::string outPath = makeOutputFilePath(m_all_matching_files[it], std::string("_proc"));
  if (m_myParams.bOutputUint16) {
    CImg<unsigned short> out16(outCimg.width(), outCimg.height(), outCimg.depth());
    cimg_forXYZ(outCimg, x, y, z) {
      float v = outCimg(x, y, z);
      if (v < 0.f) v = 0.f;
      else if (v > 65535.f) v = 65535.f;
      out16(x, y, z) = static_cast<unsigned short>(v + 0.5f);
    }
    ...
    outCimg.save_tiff(outPath.c_str(), 1);
  }
  }
#ifdef MRC
  else {
    ...
    for (int i = 0; i < m_imgParams.nz * m_myParams.z_zoom; ++i) {
      IMWrSec(ostream_no, ptr, m_imgParams.nx * m_imgParams.ny *m_myParams.zoomfact * m_myParams.zoomfact, m_out_header.mode);
      if (it == 0) {
        for (int j = 0;
...
void SIM_Reconstructor::closeFiles()
{
  if (m_myParams.bTIFF)
    ...
    #ifdef MRC
    ::IMClose(istream_no);
   // ::saveCommandLineToHeader(m_argc, m_argv, m_out_header, m_myParams);
    ::IMClose(ostream_no);
    #endif
  }
}
