...
struct ReconParams {
  ...
  /** Optional bounding-box crop applied to TIFF inputs before processing.
   *  All six values are 0-indexed, inclusive. A value of -1 means "not set";
   *  either all six must be set or none. When enabled (bCropBBox=true) the
   *  raw TIFF is cropped down to this region and nx_raw / ny / nz are replaced
   *  with the cropped dimensions; everything downstream proceeds as usual. */
  int   cropXmin, cropXmax;
